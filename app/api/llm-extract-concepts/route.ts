      ...
      try {
        ...
        for (const result of results) {
          for (const prompt of result.prompts) {
            // Use all demographics provided; default to "Unspecified" if none exist.
            const demographics = (prompt.metadata.demographics && prompt.metadata.demographics.length > 0)
            ...
            for (const response of prompt.responses) {
              ...
              if (concepts.length > 0) {
                ...
                // Add the concepts to each demographic subgroup
                demographics.forEach(demo => {
                  if (!subgroupConcepts.has(demo)) {
                    subgroupConcepts.set(demo, []);
                  }
                  subgroupConcepts.get(demo)?.push(...concepts);
                });
