  ...
  for (const promptTemplate of prompts) {
    for (const demographics of demographicGroups) {
      tasks.push(overallLimit(async () => {
        ...
        // Replace demographic placeholders if present
        ...
        // Use an inner concurrency limiter for multiple iterations per prompt.
        const innerLimit = pLimit(3);  // Adjust inner concurrency; try not to fire all iterations at once
        ...
        // Wait for all iterations for this prompt/demographic combo
